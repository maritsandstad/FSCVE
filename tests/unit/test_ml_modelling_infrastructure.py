...
def test_mlmodelinterface():
    ...
    ytrain = [18, 16, 29, 18]
    with pytest.raises(
        ml_modelling_infrastructure.UntrainedModelError,
        match=f"Model {mlinterface.model_name} has not been trained yet. Hence no prediction can be made",
    ):
    ...
    ytest = [20, 19, 24]
    ytest_predtr, maetr, msetr, rmsetr, r2tr = mlinterface.evaluate_model(
        Xtrain, ytrain
    )
    ...
    assert r2tr > r2ts
    scale_x, other_regressor = mlinterface.train_new_model_instance(
        Xtest, ytest, options={"fit_intercept": True}, update_current=False
    )
