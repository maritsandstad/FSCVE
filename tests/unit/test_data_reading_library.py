...
def test_check_data():
    ...
    with pytest.raises(
        TypeError, match="Predictor data must be in the form of a Pandas DataFrame"
    ):
        data_readying_library.check_data(predictor_list, "Hello")
    data1 = pd.DataFrame(
        data=[[59.9, 10.7, 0, 0], [52.3, 4.9, 0, 0]],
        columns=["LAT", "LON", "agb", "conifer"],
        index=["Oslo", "Amsterdam"],
    )
...
def test_cut_data_points_where_all_equal():
    ...
    data2 = pd.DataFrame(
        data=[[59.9, 10.7, 0, 1], [52.3, 4.9, 0, 0]],
        columns=["LAT", "LON", "agb", "conifer"],
        index=["Oslo", "Amsterdam"],
    )
    data1_cut, data2_cut = data_readying_library.cut_data_points_where_all_equal(
        data1, data2
    )
...
def test_fill_zeros():
    ...
    assert set(data_fill_zeros.index) == set(
        ["Oslo", "Amsterdam", "Athens", "Stockholm"]
    )
