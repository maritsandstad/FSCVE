...
def test_variable_mappings():
    assert set(forest_data_handler.variable_mapping("lat", ["LAT", "LON"])) == set((True, "LAT"))
    assert set(forest_data_handler.variable_mapping("LON", ["lat", "lon"])) == set((True, "lon")) 
...
def test_add_variables_to_forest_dataset():
    test = forest_data_handler.add_variables_to_forest_dataset("/div/no-backup-nac/users/masan/PATHFINDER/PathFinder_WP3_Task3.5_emulator/agb_and_confier_on_era5-land_resolution_test.nc", ["LAT", "LON", "ELEVATION", "TAVG", "agb_era5land", "conifer_era5land"], ["agb_era5land", "conifer_era5land"])
    assert set(test.columns) == set(["LAT", "LON", "ELEVATION", "TAVG", "agb_era5land", "conifer_era5land"])